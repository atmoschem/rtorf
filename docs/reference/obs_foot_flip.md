...
written with `obs_nc` preserve that order, but
[`terra::rast()`](https://rspatial.github.io/terra/reference/rast.html)
follows the CF / GIS convention where the *first* spatial dimension is
longitude (x-axis). Reading the file back with `terra` therefore
transposes the spatial plane, which is why a
[`t()`](https://rdrr.io/r/base/t.html) call was needed on the raster.
